...
//               <h1 className="text-[20px] md:text-2xl font-bold text-gray-600 text-center">
//                 Thống kê của <span className="text-[#FA9382]">{fullName(profile)}</span>
//               </h1>
//             </div>
...
//                 className="rounded-md p-[10px] bg-pastel-pink text-gray-600 border-[1px] border-pastel-pink"
//               >
...
//                   return <option value={key}>{enumWeek[key] + ` (${title})`}</option>;
//                 })}
//               </select>
//             </div>
//           </div>
...
//               Tổng tiền cơm: <span className="font-bold">{totalRice(groupedData)} cá</span>
//             </p>
//             <p className="text-[14px]">
//               Tổng tiền nước: <span className="font-bold">{totalWater(groupedData)} cá</span>
//             </p>
//             <p className="text-[14px]">
//               Đã chuyển: <span className="font-bold">{Math.ceil(reciptNumber)} cá</span>
//             </p>
//           </div>
//         </div>
//         <Table>
...
//               <TableHead className="bg-white rounded-md w-[150px] px-0 font-bold border-r-[1px] border-r-pastel-pink text-center">
//                 Sen
//               </TableHead>
...
//                     </p>
//                     <div className="flex items-center border-t-[1px] border-t-pastel-pink p-[10px]">
//                       <p className="w-[50%] text-center">Đồ ăn</p>
//                       <> | </> <p className="w-[50%] text-center whitespace-nowrap">Đồ uống</p>
//                     </div>
//                   </TableHead>
//                 );
//               })}
...
//                 (elm) =>
//                   elm?.user_created === userItem?.user?.id && dayjs(elm.date_start).format("YYYY-MM-DD") == currentSelect?.[0],
//               );
//               const valueRecipt = (dataReport?.[valueInput]?.name === "recipt" && dataReport?.[valueInput]?.price) || undefined;
//               return (
//                 <TableRow
//                   className={`${index % 2 == 0 ? "bg-pastel-pink/30" : "bg-white"}`}
//                   key={userItem.user.id + date + "group"}
//                 >
...
//                       (ortherItem) =>
//                         ortherItem.name === "orther-food" && dayjs(ortherItem.date_created).format("YYYY-MM-DD") == elm,
//                     );
...
//                     const valueInput = userItem.user.id + "-" + date;
//                     const valueWater =
//                       (dataReport?.[valueInput]?.name !== "recipt" && dataReport?.[valueInput]?.price) || undefined;
...
//                                     disabled
//                                     className="w-[50%] rounded-md p-[6px] text-center text-gray-600 select-none !bg-gray-200"
//                                     value={finalPrice}
//                                   ></input>
...
//                                       <p>
//                                         {item.name} - <span className="font-bold">{item.price}k</span>
//                                       </p>
//                                     </div>
//                                   ))}
//                                 </div>
...
//                             <input
//                               id={date}
//                               key={userItem.user.id + date + index + "-elm-input2"}
//                               disabled={!isAdmin}
//                               className={`rounded-md p-[6px] w-[50%] text-center bg-transparent text-gray-600 ${
//                                 isAdmin && "border-[1px] border-pastel-pink"
//                               }`}
//                               value={valueWater}
//                               defaultValue={match ? (ortherList?.price == 0 ? "" : ortherList?.price) : ""}
//                               onKeyUp={(e) => onUpdateOrder(e, userItem, ortherList, date, "orther-food")}
//                               onChange={(e) => onUpdateOrder(e, userItem, ortherList, date, "orther-food")}
//                             ></input>
//                             <div onClick={() => onPay(date, userItem, ortherList)}>
//                               {(ortherList &&
//                                 (ortherList?.confirm_paid ? (
//                                   <CircleCheckBig color="#ed4b33" strokeWidth={0.9} size={20} />
//                                 ) : (
//                                   isAdmin && <Circle color="#ed4b33" strokeWidth={0.9} size={20} />
...
//                     <input
//                       disabled={!isAdmin}
//                       className={`rounded-md p-[6px] w-[100%] text-center bg-transparent text-gray-600 ${
//                         isAdmin && "border-[1px] border-pastel-pink"
//                       }`}
