...
const Report = () => {
  ...
  const { data: reciptData, mutate: mutateRecipt } = useSWR(
    currentSelect?.[0] && `/items/recipt_84?fields=*,user.*&filter[date_start][_eq]=${dateCurrent}`,
  );
      ...
      <div className="px-[20px] md:px-[100px]">
        <div className="flex justify-start">
          <select
            ...
            className="p-[10px] bg-white text-black border-[1px] border-black"
          >
            ...
            {Object.keys(weekList).map(function (key, index) {
              ...
              return <option value={key}>{`[${title}] ` + key}</option>;
            })}
          </select>
        </div>
        <div className="flex justify-between items-center my-[20px]">
          ...
          <div>
            {admin && (
              <div
                className="border-[1px] border-black px-[30px] py-[10px] cursor-pointer bg-green-600 text-white"
                onClick={onSave}
              >
                Lưu
              </div>
        ...
        <Table>
              ...
              {currentSelect?.map((elm, index) => {
                return (
                  <TableHead className="font-bold px-0 items-center mx-auto border-r-[1px] border-r-black">
                    ...
                    <div className="flex items-center border-t-[1px] border-t-black p-[10px]">
          ...
          <TableBody className="border-l-[1px] border-l-black border-r-[1px] border-r-black">
            {groupedData?.map((userItem, index) => {
              const recipt = reciptList?.find(
                (elm) =>
                  elm?.user?.fullname === userItem?.user?.fullname &&
                  dayjs(elm.date_start).format("YYYY-MM-DD") == currentSelect?.[0],
              );
              return (
                <TableRow
                  className={`${index % 2 == 0 ? "bg-slate-200" : "bg-white"} hover:bg-unset`}
                  key={userItem.user.fullname}
                >
                  <TableCell className="font-medium text-left p-2">
                    <div className="p-[6px]">{userItem.user.fullname}</div>
                  </TableCell>
                  {currentSelect?.map((elm, index) => {
                    ...
                    return (
                      <TableCell key={weekUrl + index + "-elm-wrapper"} className="text-left p-2">
                        <input
                          key={weekUrl + index + "-elm-input1"}
                          disabled
                          className="p-[6px] w-[50%] text-center bg-transparent text-black select-none"
                          value={riceList?.length ? riceList?.length * 35 : ""}
                        ></input>
                        <input
                          key={weekUrl + index + "-elm-input2"}
                          disabled={!admin}
                          className={`p-[6px] w-[50%] text-center bg-transparent text-black ${
                            admin && "border-[1px] border-black"
                          }`}
                          defaultValue={match ? ortherList?.price : ""}
                          onChange={(e) =>
                            onUpdateOrder(
                              e,
                              userItem,
                              ortherList,
                              dayjs(elm + "T12:00:00+07:00").format("YYYY-MM-DD"),
                              "orther-food",
                            )
                          }
                        ></input>
                  ...
                  <TableCell className="text-left p-2">
                    <input
                      disabled={!admin}
                      className={`p-[6px] w-[100%] text-center bg-transparent text-black ${admin && "border-[1px] border-black"}`}
                      defaultValue={recipt?.amount && Math.ceil(recipt?.amount)}
                      onInput={(e) => onUpdateOrder(e, userItem, recipt, currentSelect[0], "recipt")}
                    ></input>
                  </TableCell>
          ...
          <TableFooter>
            <TableRow className="bg-black hover:bg-black hover:text-white text-white">
              <TableCell className="font-bold text-left" colSpan={5}>
                Tổng
              </TableCell>
